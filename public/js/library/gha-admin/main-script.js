$(function(){
	$('#main_sidebar_toggle').on('click', function(e){
		e.preventDefault();
...
(function(){
	function loadScript(nodeName, attrs){
	...
	window.ghaDeferJs = function(scripts){ //array scripts

		if(typeof scripts != "undefined" && scripts.constructor == Array) {
			for(var i in scripts) {
				switch(scripts[i].constructor) {
					case String:
						loadScript('script', {src: scripts[i]});
						break;

					case Object:
						loadScript('script', scripts[i] )
						break;
				}
			}
		}
	...
	window.ghaDeferCss = function(scripts) {
		
		if(typeof scripts != "undefined" && scripts.constructor == Array) {
			for(var i in scripts) {
				switch(scripts[i].constructor) {
					case String:
						loadScript('link', {href: scripts[i],rel: 'stylesheet'});
						break;

					case Object:
						script[i].rel = "stylesheet";
						loadScript('link', scripts[i] )
						break;
				}
			}
		}
